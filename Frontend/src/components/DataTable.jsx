        ...
        <div className="mb-3 flex items-center gap-2">
          <div className="relative w-full max-w-xs">
            ...
            <input
              ...
              placeholder="Search…"
              className="w-full rounded-lg border border-slate-200 bg-white py-2 pl-9 pr-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-400/30"
            />
          </div>
          <span className="text-xs text-slate-500">{sorted.length} record{sorted.length === 1 ? '' : 's'}</span>
        </div>
      )}
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
        <table className="w-full text-sm">
              ...
              {columns.map((col) => (
                <th
                  ...
                  className={`px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide whitespace-nowrap ${
                    col.sortable ? 'cursor-pointer select-none hover:bg-slate-500' : ''
                  }`}
                >
          ...
          <tbody>
            {pageRows.length === 0 && (
              <tr>
                <td colSpan={columns.length + (actions ? 1 : 0)} className="px-4 py-14 text-center text-slate-500">
            ...
            {pageRows.map((row, i) => (
              <tr
                key={row.id ?? i}
                className="border-b border-slate-200/60 last:border-0 hover:bg-blue-50 transition-colors"
              >
                {columns.map((col) => (
                  <td key={col.key} className="px-4 py-3 align-middle text-slate-900">
        ...
        <div className="mt-3 flex items-center justify-between text-sm text-slate-700">
          ...
          <div className="flex gap-1.5">
            <button
              ...
              disabled={pageSafe === 1}
              className="flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1.5 hover:bg-slate-200/40 disabled:opacity-40 disabled:hover:bg-transparent"
            >
